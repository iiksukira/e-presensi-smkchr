...
const RegisterFaceTeacher: React.FC = () => {
  ...
  const checkRegistrationStatus = async () => {
    try {
      const response = await api.get("/teacher/face-status");
  ...
  const submitFaceData = async (descriptors: FaceDescriptor[]) => {
    ...
    try {
      ...
      await api.post("/teacher/register-face", {
        ...
        >
          <div style={{ padding: "40px 20px" }}>
            ...
            <Button
              type="primary"
              onClick={() =>
                (window.location.href = "/teacher/self-attendance")
              }
              style={{ marginTop: 16 }}
            >
              Lanjut Presensi
...
export default RegisterFaceTeacher;
